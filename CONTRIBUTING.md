...
# /tmp git repo and asserts on the reconstructed audit log end-to-end)
bash demo.sh
```
...
## Adding detection or reconstruction logic
...
- Confirm no existing test changes outcome. Detection is token/structure-based;
  changes to the sensitive-name list, the content sniff, or a redaction regex
  must come with a test that exercises the new path.
...
The core invariant: the audit store (`.alog/`) must never contain a secret's
bytes in cleartext. Sensitive files are recorded as a salted digest only, the
store is created `0700/0600`, and it drops its own `.gitignore`. Any change near
snapshotting or the content-addressed store must keep `demo.sh`'s secret-at-rest
assertions passing.
